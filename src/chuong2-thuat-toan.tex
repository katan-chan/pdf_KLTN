...
\begin{enumerate}[topsep=4pt, itemsep=4pt, parsep=0pt]
    \item Bảo toàn liên thông qua $A$. Nếu đỉnh $u$ liên thông với phần còn lại của $V$ trong $G$ thì $u$ cũng liên thông trong $G_\lambda$. Cụ thể, mọi đỉnh có $d_u \geq 1$ trong $G$ đều có $(D_\lambda)_{uu} \geq 1 > 0$, do đó không bị cô lập trong $G_\lambda$ kể cả khi $d_u^{(M)} = 0$.
...
\begin{proof}
\emph{Phần (1).} Theo định nghĩa của $D_\lambda$,
\[
    (D_\lambda)_{uu} = \sum_{v \in V} (W_\lambda)_{uv} = \sum_{v \in V} \bigl(A_{uv} + \lambda W^{(M)}_{uv}\bigr) = d_u + \lambda \, d_u^{(M)},
\]
trong đó $d_u = \sum_v A_{uv}$ là bậc trong $G$ và $d_u^{(M)} = \sum_v W^{(M)}_{uv}$ là bậc trọng số trong $G_M$. Vì $W^{(M)} \geq 0$ và $\lambda \geq 0$, số hạng $\lambda d_u^{(M)} \geq 0$. Do đó $(D_\lambda)_{uu} \geq d_u$. Khi $u$ liên thông với phần còn lại của $V$ trong $G$, tức $d_u \geq 1$, ta có $(D_\lambda)_{uu} \geq 1 > 0$. Hệ quả, $u$ không cô lập trong $G_\lambda$ kể cả khi $d_u^{(M)} = 0$, trái với tình huống trong $G_M$ thuần.
