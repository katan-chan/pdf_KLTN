...
\end{itemize}
Ngay cả khi đã cố định một tiêu chí $f$, việc tìm phân hoạch tối ưu $f$ vẫn là một bài toán khó. Lực lượng của $\mathcal{P}(V)$ tăng theo cấp số mũ với $n$ và được đếm bởi số Bell $B_n$, nên duyệt toàn bộ là không khả thi với đồ thị quy mô lớn. Hơn nữa, hai bài toán cụ thể là cực đại modularity và cực tiểu normalized cut đều thuộc lớp NP-khó \cite{newman2006, shi2000}.
...
Cho phân hoạch $\mathcal{C} = \{C_1, \ldots, C_K\}$ với $c_u$ ký hiệu cụm chứa đỉnh $u$, và $\delta(c_u, c_v)$ là Kronecker delta, bằng $1$ nếu $c_u = c_v$ và bằng $0$ nếu khác. Modularity được định nghĩa
\[
...
\end{equation}
Ma trận $S$ là ma trận assignment chịu ba ràng buộc, mỗi ràng buộc thể hiện một tính chất bắt buộc của phân hoạch.
