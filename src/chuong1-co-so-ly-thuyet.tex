...
Mô hình ngẫu nhiên cùng dãy bậc cho kỳ vọng cạnh giữa hai đỉnh $u$ và $v$ bằng $d_u d_v / (2m)$. Ma trận modularity được định nghĩa
\[
    B = A - \frac{d d^\top}{2m},
\]
...
Cho phân hoạch $\mathcal{C} = \{C_1, \ldots, C_K\}$ với $c_u$ ký hiệu cụm chứa đỉnh $u$. Modularity được định nghĩa
\[
...
\textbf{Bài toán tối ưu.} Bài toán phát hiện cộng đồng theo modularity được phát biểu như sau. Hàm mục tiêu là vết của ma trận $S^\top B S$, đại diện cho tổng modularity tích lũy trên các cụm:
\begin{equation}
\label{eq:modularity-objective}
    \max_{S \in \mathbb{R}^{n \times K}} \; \mathrm{Tr}(S^\top B S).
\end{equation}
Ma trận $S$ chịu ba ràng buộc, mỗi ràng buộc thể hiện một tính chất bắt buộc của phân hoạch.
...
\begin{remark}[Resolution limit]
\label{rem:resolution-limit}
Modularity có một giới hạn nội tại liên quan đến kích thước cụm, được phát hiện và phân tích bởi Fortunato và Barthélemy năm 2007 \cite{fortunato2007}. Nguyên nhân nằm ở chính mô hình ngẫu nhiên null trong định nghĩa $B$: kỳ vọng cạnh $d_u d_v / (2m)$ được chuẩn hóa theo tổng cạnh toàn cục $m$, do đó một cụm có mật độ nội bộ tuyệt đối thấp vẫn bị xem là không khác biệt đáng kể với phần còn lại, ngay cả khi mật độ tương đối của nó cao hơn nhiều so với mặt bằng chung. Cụ thể, tồn tại một ngưỡng
\[
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{resolution_limit.pdf}
    \caption[Resolution limit trên vòng $16$ tam giác $K_3$.]{Resolution limit trên vòng $16$ tam giác $K_3$. Phân hoạch tự nhiên (a) đúng với cấu trúc nhưng cho $Q$ thấp hơn phân hoạch gộp cặp (b).}
    \label{fig:resolution-limit}
\end{figure}

Hình~\ref{fig:resolution-limit} minh họa hiện tượng này trên vòng $16$ tam giác $K_3$ nối với nhau bằng một cạnh giữa hai tam giác kề. Phân hoạch tự nhiên với mỗi tam giác là một cụm cho $Q = 0{,}6875$, trong khi phân hoạch gộp từng cặp tam giác kề nhau cho $Q = 0{,}7500$. Modularity ưu tiên cấu hình gộp dù cấu hình này rõ ràng kém hơn về mặt cấu trúc cộng đồng. Với $m = 64$ ta có $\gamma(m) \approx 5{,}66$, mỗi tam giác chỉ có $\ell = 3 < \gamma(m)$, đúng theo dự đoán của ngưỡng phân giải.
\end{remark}
...
\begin{theorem}[Tính chất biến phân Courant--Fischer]
...
Mục~\ref{sec:problem} đã trình bày bài toán phát hiện cộng đồng dưới góc nhìn cạnh: chất lượng phân hoạch được đo qua $\mathrm{cut}$, $\phi$, $Q$, đều là các đại lượng dựa trên cấu trúc cạnh đơn lẻ. Tuy nhiên trong nhiều mạng thực, đặc biệt là mạng xã hội, cấu trúc bậc cao mang theo tín hiệu cộng đồng mạnh hơn cả cạnh đơn lẻ. Mục này hình thức hóa khái niệm đó thông qua \emph{motif} và \emph{đồ thị Higher Order}, đồng thời chứng tỏ rằng tam giác $K_3$ là motif then chốt cho lớp đồ thị xã hội thực tế thông qua hệ số phân cụm.
...
\begin{definition}[Hệ số phân cụm cục bộ]
\label{def:local-cc}
Hệ số phân cụm cục bộ của đỉnh $u$ là
\[
...
Cho cụm $C_j$ trong phân hoạch $\mathcal{C}$, đặt
\[
    \mathrm{CC}(C_j) = \frac{1}{|C_j|} \sum_{u \in C_j} cc(u).
\]
Hai dạng tổng hợp trên toàn phân hoạch:
\begin{align*}
    \overline{\mathrm{CC}}(\mathcal{C})
        &= \frac{1}{K} \sum_{j=1}^K \mathrm{CC}(C_j), \\[4pt]
...
Đại lượng $\overline{\mathrm{CC}}(\mathcal{C})$ là trung bình cộng các $\mathrm{CC}(C_j)$ trên các cụm với trọng số đều, còn $\overline{\mathrm{CC}}_w(\mathcal{C})$ là trung bình có trọng số tỷ lệ với kích thước cụm.

\textbf{Vì sao cần $\overline{\mathrm{CC}}_w$.} Đại lượng $\overline{\mathrm{CC}}(\mathcal{C})$ chỉ đo mật độ tam giác bên trong từng cụm và bỏ qua hoàn toàn kích thước của cụm. Hệ quả lý thuyết là một cụm chỉ gồm ba đỉnh nối thành tam giác $K_3$ luôn cho $\mathrm{CC}(C) = 1$, và đóng góp của cụm này vào $\overline{\mathrm{CC}}$ ngang bằng đóng góp của một cụm hàng trăm hay hàng nghìn đỉnh. Nói rộng hơn, mọi cụm bất kỳ đều đóng góp đồng đều một lượng $\tfrac{1}{K}$ vào $\overline{\mathrm{CC}}$, không phân biệt cụm đó chứa ba đỉnh hay phần lớn đồ thị. Một phân hoạch sinh ra nhiều cụm rất nhỏ với hệ số phân cụm cao do đó có thể đạt $\overline{\mathrm{CC}}$ rất gần $1$ mặc dù về mặt cấu trúc cộng đồng, các cụm cỡ ba đỉnh thường không có ý nghĩa.

Đại lượng $\overline{\mathrm{CC}}_w(\mathcal{C})$ được thiết kế để khắc phục điểm yếu này bằng cách thay trọng số đều bằng trọng số tỉ lệ với $|C_j|$. Mỗi cụm khi đó đóng góp vào trung bình theo đúng số đỉnh nó chứa, nên cụm $K_3$ chỉ đóng góp ba đỉnh trên tổng $n$, không thể kéo trung bình lên cao chỉ vì có $\mathrm{CC} = 1$. Công thức rút gọn $\overline{\mathrm{CC}}_w(\mathcal{C}) = \frac{1}{n} \sum_{u \in V} cc(u)$ cho thấy đại lượng này thực chất là trung bình hệ số phân cụm cục bộ trên toàn bộ đỉnh, không phụ thuộc cách phân hoạch chia cụm. Vì lý do đó, $\overline{\mathrm{CC}}_w$ được dùng làm đại lượng đánh giá chính trong khoá luận; $\overline{\mathrm{CC}}$ được tính kèm để phát hiện hiện tượng phân hoạch tạo ra cụm rất nhỏ làm thiên lệch chỉ số.

\textbf{Bằng chứng thống kê cho vai trò của tam giác.} Việc chọn tam giác $K_3$ làm motif không phải lựa chọn tùy ý mà được công nhận rộng rãi trong tài liệu phân tích mạng nhờ bằng chứng thống kê trên nhiều lớp mạng thực tế khác nhau. Watts và Strogatz \cite{watts1998} là những người đầu tiên chỉ ra rằng nhiều mạng thực, từ mạng diễn viên đồng diễn, mạng kết nối synapse trong hệ thần kinh của giun \emph{C. elegans} đến mạng lưới điện, có hệ số phân cụm trung bình lớn hơn rất nhiều so với mô hình ngẫu nhiên có cùng số đỉnh và cạnh. Kết quả này được Newman tổng hợp lại trong khảo sát \cite{newman2003structure} cho nhiều lớp mạng thực: mạng cộng tác khoa học, mạng email, mạng web, mạng xã hội. Hệ số phân cụm trung bình trên các mạng này nằm trong khoảng $0{,}1$--$0{,}8$, trong khi mật độ cạnh thường chỉ ở mức $10^{-3}$ trở xuống, lớn hơn dự đoán ngẫu nhiên từ một đến nhiều bậc.
...
Ma trận $W^{(M)}$ đối xứng và không âm. Do đó $G_M$ là đồ thị có trọng số vô hướng hợp lệ, và mọi kết quả phổ phát biểu cho lớp đồ thị này, bao gồm bất đẳng thức Cheeger và các phép nới lỏng phổ của NCut hay modularity, áp dụng được cho $G_M$.
\end{proposition}

Tương tự đồ thị gốc, ta định nghĩa motif degree $d_u^{(M)} = \sum_{v \in V} W^{(M)}_{uv}$ và motif volume $\mathrm{vol}_M(S) = \sum_{u \in S} d_u^{(M)}$. Đại lượng cắt theo motif là
\[
...
Phân cụm phổ trên $W^{(M)}$ thu được nhát cắt có $\phi_M$ gần tối ưu, với cùng dạng cận như bất đẳng thức Cheeger cổ điển. Tính chất này là hệ quả trực tiếp của Mệnh đề~\ref{prop:gm-valid-weighted-graph} và bất đẳng thức Cheeger áp dụng cho $G_M$.
\end{theorem}
